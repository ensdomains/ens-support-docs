...
# Coinbase wallet
Coinbase Wallet is a self-custody wallet, which allows users to manage their own private keys to the wallet and crypto. This standalone wallet app does not require a Coinbase account and is not to be confused with the Coinbase Exchange app.
...
<BrowserWindow url="app.ens.domains">
  <Image img="/img/coinbase-wallet-1.png"/>
</BrowserWindow>
...
Coinbase Wallet may rely on OpenSea NFT data endpoints to display information about ENS names. If OpenSea does not have the latest metadata, try refreshing on their platform. See #OpenSea-refresh- metadata
