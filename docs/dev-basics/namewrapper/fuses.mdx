...
So even if the DNS TLD owner "Locks" that TLD in the ENS Name Wrapper, **if** that TLD were to ever change ownership on the DNS side, then the new owner would be able to override control of that TLD on the ENS side, unwrap it, and replace/revoke all 2LDs.
