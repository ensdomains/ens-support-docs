...
For Coinbase wallet (not the Coinbase exchange app!), go into `settings` and look for the `Active dapp network` option. Scroll down until you see the `ETH TESTNETS` section. Now select `Görli` (yes it's the same are Goerli).
...
Search for an ENS name you'd like to register in [ENS App Manager V3 preview site](https://alpha.ens.domains).

<BrowserWindow>
  <Image img="/img/tutorials/test-v3-manager-4.png"/>
</BrowserWindow>

From here, you can test the new registration process. You will be able to set up records, Primary address, and upload an avatar directly all during the registration process. You can also explore advanced features like setting up NameWrapper permissions. Have fun exploring!
