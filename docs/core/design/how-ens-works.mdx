...
sequenceDiagram
    activate User Code
    activate ENS Registry

    rect rgba(0,255,0,0.1)
    note over User Code,ENS Registry: 1. Retrieve Resolver from Registry
    User Code->>ENS Registry: resolver('foo.eth')
    ENS Registry->>User Code: 0x12342...
    deactivate ENS Registry
    end

    activate Resolver @ 0x1234...

    rect rgba(0,255,255,0.1)
    note over User Code,Resolver @ 0x1234...: 2. Retrieve Address from Resolver
